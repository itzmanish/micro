...
import (
	...
	tmpl "github.com/itzmanish/micro/v2/internal/template"
	"github.com/itzmanish/micro/v2/internal/usage"
...
func protoComments(goDir, alias string) []string {
	return []string{
		...
		"go get github.com/itzmanish/micro/v2/cmd/protoc-gen-micro",
...
func copyAPIProto(c config) (string, error) {
	...
	if err != nil {
		return "", errors.New("Unable to find go-micro version. Please try `go get github.com/itzmanish/go-micro/v2`")
	}
	newestDir := ""
	...
	input, err := ioutil.ReadFile(fmt.Sprintf("%s/pkg/mod/github.com/itzmanish/go-micro/%s/api/proto/api.proto", basedir, newestDir))
	if err != nil {
		return "", err
	}
